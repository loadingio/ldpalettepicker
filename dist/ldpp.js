...
ldPalettePicker = function(node, opt){
  ...
  el.pn = {
    vw: ld$.find(root, '.panel[data-panel=view]', 0),
    mp: ld$.find(root, '.panel[data-panel=mypal]', 0),
    ed: ld$.find(root, '.panel[data-panel=edit]', 0)
  };
  el.pn.vwin = ld$.find(el.pn.vw, '.inner', 0);
  el.ed = {
    picker: ld$.find(el.pn.ed, '.ldColorPicker', 0),
    pal: ld$.find(el.pn.ed, '.palette', 0),
    colors: ld$.find(el.pn.ed, '.palette .colors', 0),
    hex: ld$.find(el.pn.ed, 'input[data-tag=hex]', 0),
    sel: ld$.find(el.pn.ed, 'select', 0),
    cfgs: ld$.find(el.pn.ed, '.config')
  };
  content = {
    build: function(p, tgt){
      var rows, lines, i$, step$, to$, i, line, j$, to1$, j, this$ = this;
      p == null && (p = []);
      tgt == null && (tgt = 'vw');
      if (opt.useClusterizejs) {
        ...
        return new Clusterize({
          rows: lines,
          contentElem: el.pn[tgt + "in"],
          scrollElem: el.pn[tgt]
        });
      } else {
        return el.pn[tgt].innerHTML = p.map(function(it){
          return this$.html(it);
        }).join('');
      }
    },
  ...
  ['rgb-r', 'rgb-g', 'rgb-b', 'hsl-h', 'hsl-s', 'hsl-l', 'hcl-h', 'hcl-c', 'hcl-l'].map(function(it){
    return function(t){
      ...
      return $(ld$.find(root, ".ion-slider[data-tag=" + t + "]", 0)).ionRangeSlider((ref$ = irsOpt[t], ref$.onChange = function(e){
        var c;
        ldcp._slider = t;
        c = ldcp.getColor(v[0]);
        c[v[1]] = e.from;
  ...
  editUpdate = function(c){
    ...
    return ['rgb-r', 'rgb-g', 'rgb-b', 'hsl-h', 'hsl-s', 'hsl-l', 'hcl-h', 'hcl-c', 'hcl-l'].map(function(t){
      ...
      return $(ld$.find(root, ".ion-slider[data-tag=" + t + "]", 0)).data("ionRangeSlider").update({
        from: v
      });
    });
  };
